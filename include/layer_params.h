...
struct ConvLayerParams {
	...
	int input_h, input_w;
	int output_channels, output_h, output_w;
	int kernel_size;
	...
	void convLoadWeights(size_t data_type_size, FILE *wfp, int bn, cudaStream_t stream);
};
...
struct FCLayerParams {
	...
	void cnmemFreeDerivatives(cudaStream_t stream);
	void cnmemFreeDerivativesLocked(NeuralNet *net, cudaStream_t stream);
	void fcLoadWeights(size_t data_type_size, FILE *wfp, cudaStream_t stream);
};
...
struct PoolingLayerParams {
	cudnnTensorDescriptor_t input_tensor;
	cudnnTensorDescriptor_t output_tensor;
	int input_h,input_w,input_channels;
	int stride, padding,size;
	PoolingMode mode;
