...
enum PoolingMode {POOLING_MAX, POOLING_AVERAGE, POOLING_AVERAGE_COUNT_INCLUDE_PADDING, POOLING_AVERAGE_COUNT_EXCLUDE_PADDING};
enum ActivationMode {SIGMOID, RELU,LEAKY_RELU, TANH, CLIPPED_RELU, ELU, ACTIVATION_NONE};
enum UpdateRule {SGD};
...
struct ConvDescriptor {
	...
	void initializeValues(int input_channels, int output_channels, int kernel_h, int kernel_w, int input_h, int input_w, 
						int pad_h, int pad_w, int stride_x, int stride_y, int bn, ActivationMode activation_mode = ACTIVATION_NONE, double actv_coef = 1.0);	
};
