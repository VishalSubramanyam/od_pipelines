...
NeuralNet::NeuralNet(std::vector<LayerSpecifier> &layers, DataType data_type, int batch_size, TensorFormat tensor_format, 
	...
	//fprintf(stderr,"Image data filenae is %s\n",imgfname);


	//Open weight file- tiny yolo-
	...
	int major;
    	...
    	int revision;
	int iseen;
    	fread(&major, sizeof(int), 1, this->wfp); 
	fread(&minor, sizeof(int), 1, this->wfp); 
	fread(&revision, sizeof(int), 1, this->wfp);
    	fread(&iseen, sizeof(int), 1, this->wfp); 
...
//---	dlayer_input = (void **)malloc((num_layers + 1) * sizeof(void *));
	...
	for (int i = 0; i < num_layers; i++) {
		...
		if (layers[i].type == CONV) {
			...
			((ConvLayerParams *)params[i])->initializeValues(cudnn_handle, user_params, this->data_type, batch_size, this->tensor_format,data_type_size, current_output_size, update_rule);
		}
			...
			((FCLayerParams *)params[i])->initializeValues(user_params, batch_size, this->tensor_format, this->data_type,current_output_size, update_rule);
		}
			...
			((DropoutLayerParams *)params[i])->initializeValues(cudnn_handle, user_params, this->data_type, batch_size,this->tensor_format, current_output_size);
			...
			((BatchNormLayerParams *)params[i])->initializeValues(user_params, this->data_type, this->tensor_format, batch_size,current_output_size, update_rule);
			...
			((ActivationLayerParams *)params[i])->initializeValues(user_params, this->data_type, this->tensor_format,batch_size, current_output_size);
		}
...
//#	std::cout << "actual_total_consume(MB): " << (init_free_bytes - (free_bytes + 600 * 1024 * 1024 - max_consume)) / (1.0 * 1024 * 1024) << std::endl;
	...
	*/
	...
	// }
        ...
        cur_prefetch_layer=0;
	event_mem_op_start = (cudaEvent_t *)malloc((num_layers + 1) * sizeof(cudaEvent_t));
	event_mem_op_end = (cudaEvent_t *)malloc((num_layers + 1) * sizeof(cudaEvent_t));
    	for(int i=0; i <= num_layers; i++){
       		checkCudaErrors(cudaEventCreate(&event_mem_op_start[i]));
      	 	checkCudaErrors(cudaEventCreate(&event_mem_op_end[i]));
        }
...
void NeuralNet::deallocateSpace(){
	for(int i=0;i<num_layers+1;i++)
	{

		threadFreeLayerInputFunction(i);
	}	
	fclose(wfp);
}
...
bool NeuralNet::simulateNeuralNetworkMemory(vDNNConvAlgoPref algo_pref, bool hard, size_t &exp_max_consume, size_t &max_consume) {
	...
	//--std::cerr << "NNSimulation: Initial Used space(MB): " << space_tracker.getConsumed() << std::endl;
...
//--		resetPrefetched();
		...
		checkCNMEMSim(cnmemMalloc(&layer_input[0], layer_input_size[0] * data_type_size, NULL), layer_input_size[0] * data_type_size, max_consume, free_bytes, checkCNMEM(cnmemFinalize()); continue, out_of_memory);
		...
		for (int i = 0; i < num_layers; i++) {
			...
			checkCNMEMSim(cnmemMalloc(&layer_input[i + 1], layer_input_size[i + 1] * data_type_size, NULL),layer_input_size[i + 1] * data_type_size, max_consume, free_bytes, break, out_of_memory);
			...
			if (layer_type[i] == CONV) {
				...
				checkCNMEMSim(cnmemMalloc(&cur_workspace, cur_workspace_size, NULL),cur_workspace_size, max_consume, free_bytes, break, out_of_memory);
			...
			if (layer_type[i] == CONV) {
				checkCNMEMSim(cnmemFree(cur_workspace, NULL),cur_workspace_size, max_consume, free_bytes, break, out_of_memory);
...
//--			if (to_offload[i]) {
				checkCNMEMSim(cnmemFree(layer_input[i], NULL),layer_input_size[i] * data_type_size, max_consume, free_bytes, break, out_of_memory);
...
//--comment backpropogation simulation
//--
/*
		checkCNMEMSim(cnmemMalloc(&dlayer_input[num_layers], batch_size * num_classes * data_type_size, NULL),layer_input_size[num_layers] * data_type_size, max_consume, free_bytes, checkCNMEM(cnmemFinalize()); continue, out_of_memory);
...
*/
//---commenting of backpropogatin ends here
