...
{
	softmax_kernel<<<cuda_gridsize(batch * groups), BLOCK1,0, stream >>>(input, n, batch, batch_offset, groups, group_offset, stride, temp, output);
	check_error(cudaPeekAtLastError());
}
...
void copy_gpu_offset(int N, float * X, int OFFX, int INCX, float * Y, int OFFY, int INCY, cudaStream_t stream) {
    copy_kernel<<<cuda_gridsize(N), BLOCK1,0, stream>>>(N, X, OFFX, INCX, Y, OFFY, INCY);
...
int entry_index(int batch, int location, int entry,int w, int h, int outputs, int coords,int classes) {
    int n = location / (w*h);
    int loc = location % (w*h);
    //printf("batch %d n  %d loc %d entry %d coords %d classes %d\n", batch, n,loc,entry, coords,classes);
    return batch*outputs + n*w*h*(coords+classes+1) + entry*w*h + loc;
}
//Region layer processing 
void forward_region_layer_gpu(int inputs, int outputs, float *input, int batch,int h, int w, int nm, int classes, int coords, float *output, cudaStream_t cs) {
    int background=0;
    int softmax=1;
    int softmax_tree=0;
    int b,n;
    ...
    for (b = 0; b < batch; ++b){
        for(n = 0; n < nm; ++n){
            int index = entry_index(b, n*w*h, 0,w,h,outputs,coords,classes);
	    //printf("\n first index is %d ",index);
            activate_array_gpu1(output + index, 2*w*h, LOGISTIC,cs);
            if(coords > 4){
                index = entry_index(b, n*w*h, 4,w,h,outputs,coords,classes);
	    	//printf("index is %d ",index);
                activate_array_gpu1(output + index, (coords - 4)*w*h, LOGISTIC,cs);
            }
	    index = entry_index(b, n*w*h, coords, w,h,outputs,coords,classes);
	    //printf("\nindex is %d ",index);
            if(!background) activate_array_gpu1(output + index, w*h, LOGISTIC,cs);
	    index = entry_index(b, n*w*h, coords + 1, w,h,outputs,coords,classes);
	    //printf("\nindex is %d ",index);
            if(!softmax && !softmax_tree) activate_array_gpu1(output + index, classes*w*h, LOGISTIC,cs);
        }
    }
    if (softmax) {
        int index = entry_index( 0, 0, coords + !background,w,h,outputs,coords,classes);
	//printf(" softmax index is %d ",index);
        softmax_gpu(input + index, classes + background, batch*nm, inputs/nm, w*h, 1, w*h, 1, output + index,cs);
    }
...
void get_region_detections(RegionLayerParams *p, int w, int h, float thresh, int relative, detection *dets, float *output, int outputs, int netw, int neth)
{
    int i,j,n, background=0;
    float biases[]={1.08,1.19,3.42,4.41,6.63,11.38,9.42,5.11,16.62,10.52};
    float *predictions = output;
    //printf("Prediction %f", predictions[0]);
    if(predictions ==NULL) printf("Prediction NULL error");
    for (i = 0; i < p->width*p->height; ++i){
        ...
        for(n = 0; n < p->num; ++n){
            ...
            int obj_index  = entry_index( 0, n*p->width*p->height + i, p->coords,    p->width,p->height,outputs,p->coords,p->classes);
            int box_index  = entry_index( 0, n*p->width*p->height + i, 0,    p->width,p->height,outputs,p->coords,p->classes);
            int mask_index = entry_index( 0, n*p->width*p->height + i, 4,    p->width,p->height,outputs,p->coords,p->classes);
	    //printf("OBJ index is %d",obj_index);
            float scale = background ? 1 : predictions[obj_index];
            dets[index].bbox = get_region_box(predictions, biases, n, box_index, col, row, p->width, p->height, p->width*p->width);
            dets[index].objectness = scale > thresh ? scale : 0;
            if(dets[index].mask){
                for(j = 0; j < p->coords - 4; ++j){
                    dets[index].mask[j] = output[mask_index + j*p->width*p->height];
                }
            }

            int class_index = entry_index(0, n*p->width*p->height + i, p->coords + !background,p->width,p->height,outputs,p->coords,p->classes);
;
            if(dets[index].objectness){
              for(j = 0; j < p->classes; ++j){
                  int class_index = entry_index(0, n*p->width*p->height + i, p->coords + 1 + j,p->width,p->height,outputs,p->coords,p->classes);
;
    ...
    correct_region_boxes(dets, p->width*p->height*p->num, w, h, netw, neth, relative);
}


void fill_network_boxes(RegionLayerParams *p, int w, int h, float thresh, int relative, detection *dets, float *output, int outputs, int input_w, int input_h)
{
    get_region_detections(p,w, h, thresh, relative, dets, output, outputs,input_w, input_h);
