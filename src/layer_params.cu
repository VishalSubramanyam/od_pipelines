...
void ConvLayerParams::initializeValues(cudnnHandle_t cudnn_handle, ConvDescriptor *user_params, cudnnDataType_t data_type, 
	...
	checkCUDNN(cudnnSetTensor4dDescriptor(input_tensor, tensor_format, data_type, batch_size, user_params->input_channels, user_params->input_h, user_params->input_w));


	checkCUDNN(cudnnSetFilter4dDescriptor(filter_desc, data_type, tensor_format, user_params->output_channels, user_params->input_channels, user_params->kernel_h, user_params->kernel_w));
...
void ConvLayerParams::allocateSpace(curandGenerator_t curand_gen, cudnnDataType_t data_type, size_t data_type_size, 
	...
	//-- Allocate device memory for scales, rolling variance and rolling mean;
...
void ConvLayerParams::convLoadWeights(size_t data_type_size, FILE *wfp, int bn, cudaStream_t stream_memory){
	...
	if(bn==1){
	    s2=fread(scales_cpu,sizeof(float),nb,wfp);
	    s3=fread(rolling_mean_cpu,sizeof(float),nb,wfp);
	    s4=fread(rolling_variance_cpu,sizeof(float),nb,wfp);
	    /*if(s2 !=nb || s3 !=nb || s4 != nb){
		fprintf(stderr,"Error in the amount of data read from file-  rollling mean and variance");
		exit(0);
	    }*/	    
	}
	s5=fread(temp_W, sizeof(float),nW,wfp);//error coming here
	/*if(s1!=nb || s5!=nW){
...
void PoolingLayerParams::initializeValues(PoolingDescriptor *user_params, cudnnDataType_t data_type, cudnnTensorFormat_t tensor_format, 
	...
	stride=user_params->stride_x;
	padding=user_params->pad_h;
	size=user_params->kernel_h;	
