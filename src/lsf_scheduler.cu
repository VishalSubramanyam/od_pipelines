...
using namespace std;
void LSFScheduler::dispatch(Operation *tp, int index) {
...
void LSFScheduler::loadDeadlines() {
    for (int i = 0; i < no_of_pipelines; i++) {
        LSFScheduler::deadline[i] = 80.0;
    }
}
...
timeMS sumOfExecutionTimes(Operation *op) {
    timeMS sum = 0.0;
...
void LSFScheduler::execute(Operation *tp, int index) {
    if (tp->op_type == 'c') {
        ...
        if (tp->op_layer == 0) {
            ...
            tp->model->prefetchWeights(tp->op_layer,
                                       LSFScheduler::stream[index]);
        }
    }
    cudaStreamAddCallback(stream[index], )
}
void LSFScheduler::start() {
    assert(!HEAD.empty());
    ...
    // slack1 = deadline1 - currentTime - sumOfOtherOperations -
    // remainingTimeInCurrentOperation;
}

void CUDART_CB LSFScheduler::perform_lsf(cudaStream_t stream,
...
void LSFScheduler::setHEAD(vector<InputOperation *> &dags) {
    for (int i = 0; i < no_of_pipelines; i++) {
        HEAD[i] = dags[i];
    }
}